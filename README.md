...
- 🔖 **文件溯源码**：下载时在文件副本中注入「下载者 ID + 时间」标记
  ...
  外泄文件可用 `/trace` 反查下载者；**上传者可按文件选择是否启用**
...
| `/reset_server` | 初始化本服务器（需 3 名管理员同意） | 管理员 |
...
- 🐛 修复图片溯源水印的一系列问题：
  - 水印处理改为线程执行，不再阻塞事件循环（修复下载消息延迟/卡顿）
  - 大 PNG 重编码提速约 5 倍（compress_level 调整）
  - 无 freetype / 老版本 Pillow 时字体回退正确放大，水印不再小到不可见
  - 超过 3000 万像素的图片跳过水印，避免卡顿和内存爆炸
  - 水印输出超过 20MB 时放弃注入，保证文件始终能送达
  - 保存格式白名单校验，未知图片格式不再静默失败

### v1.5.0

- ✨ 上传准备页新增「🔖 溯源开关」：上传者可按文件选择是否启用下载溯源标记
  （默认开启）；关闭溯源会写入审计日志与管理日志，`/fileinfo` 显示开关状态
