...
- 🗳️ **一键整理**：`/organize` 预扫描后发起管理员投票（整理当前子区需 2 名管理员同意，
  ...
  **投票通过后管理员可勾选本次跳过的频道再确认执行**，还可一键写入持久黑白名单
- 🛡️ **整理黑白名单**：`/organize_filter` 配置永久生效的频道名单——
  黑名单模式（整理名单以外全部）或白名单模式（只整理名单内），闲聊频道永不误扫
...
| `/stats` | 仓库统计 | 管理员 |
| `/organize` | 预扫描 + 投票整理存储文件（子区 2 票 / 全服 3 票），通过后可确认范围 | 管理员 |
| `/organize_filter` | 配置整理的频道黑白名单（永久生效） | 管理员 |
