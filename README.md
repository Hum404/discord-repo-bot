...
- 🗳️ **一键整理**：`/organize` 预扫描后发起管理员投票（整理当前子区需 2 名管理员同意，
...
- 🎫 **申诉工单**：被风控成员 `/appeal` 提交申诉，Bot 发送到 `/ticket_channel`
  指定的频道并自动 @所有管理员角色，集齐 2 名管理员同意方可解封（2 票拒绝则驳回）；
...
- 🛡️ **管理日志**：`/storage_mode`、`/log_channel`、`/organize`、`/delete` 等
  ...
  （未设置时跟随审计日志频道）
- 🔖 **文件溯源码**：下载时在文件副本中以不可见形式注入加密溯源标记
  （无法读取、篡改即失效），外泄文件可用 `/trace` 反查下载者；
  **上传者可按文件选择是否启用**
...
| `/stats` | 仓库统计 | 管理员 |
| `/organize` | 预扫描 + 投票整理存储文件（子区 2 票 / 全服 3 票），通过后确认黑白名单范围 | 管理员 |
...
- 🔐 **溯源标记机制重构**：
  - 标记内容不再明文可读，无法被读取、伪造或转移
  - 标记一旦被篡改即失效，可追溯性显著增强
  - 标记改为完全隐形注入，图片不再有可见水印
  - `/trace` 溯源查询支持交叉验证与置信度分级
  - 兼容识别旧版明文标记（标注为「可伪造」，可对照下载记录验证）
  - 加密实现细节不予公开，以保障溯源机制的有效性

### v1.10.0

- ✨ **角色卡溯源**：下载 PNG / JSON 角色卡时不再破坏卡片——
  - **PNG 角色卡**（含 `chara` tEXt/zTXt 块，如 SillyTavern 卡）：在 IEND 前注入独立的
    `trc` tEXt 元数据块，**不动像素、不重编码、不丢 chara 数据、不打可见水印**，
    导入角色卡软件完全不受影响
  - **JSON / JSON 角色卡**：解析后写入 `extensions.trc` 字段（V2 卡写入
    `data.extensions.trc`），不再用零宽字符破坏 JSON 格式，文件保持可解析
  - 非角色卡的图片仍走右下角可见水印，其他文本仍走零宽字符，行为不变
- ✨ `/trace` 反查增强：新增明文字节扫描，PNG tEXt 块、JSON 字段、ZIP comment 中的
  `TRC|id|ts` 标记均可直接命中定位；PNG 角色卡未命中时提示「无标记」而非「看右下角」
